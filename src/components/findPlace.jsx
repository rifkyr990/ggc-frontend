...
export default function FindPlace() {
    ...
    const fetchPerumahan = async ({ search = "", page = 1 }) => {
        try {
    ...
    useEffect(() => {
        const searchQuery = `${lookingFor} ${location} ${propertyType} ${price}`.trim();
        setLoading(true);
        fetchPerumahan({ search: searchQuery, page: currentPage })
            .then((res) => {
    ...
    return (
        <div className="bg-[#f5f5f7] min-h-[140vh] flex flex-col items-center justify-start py-16 px-6 md:px-32">
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-16 w-full max-w-7xl">
                {/* Filter select components (unchanged styling) */}
                {[
                    { label: "Looking For", state: lookingFor, setter: setLookingFor, options: ["Dijual", "Disewa"] },
                    { label: "Location", state: location, setter: setLocation, options: [...new Set(properties.map(p => p.lokasi).filter(Boolean))] },
                    { label: "Property Type", state: propertyType, setter: setPropertyType, options: ["Rumah", "Apartemen", "Ruko"] },
                    { label: "Price", state: price, setter: setPrice, options: ["<500jt", "500jt-1M", ">1M"] }
                ].map(({ label, state, setter, options }, idx) => (
                    <div key={idx} className="flex-1 flex flex-col">
                        <label className="mb-2 text-xs font-light text-gray-700 tracking-wide">{label}</label>
                        <select
                            value={state}
...
>
        ...
        <div className="p-6 flex-1 flex flex-col justify-between gap-2">
            <div className="font-semibold text-lg text-gray-900 mb-1">{property.nama}</div>
            <div className="flex flex-wrap gap-2 text-xs mb-2">
                <span className="bg-[#BFA14A]/10 text-[#BFA14A] px-2 py-1 rounded-full">{property.lokasi}</span>
            ...
            <div className="flex justify-between text-gray-500 text-sm border-t pt-2 mt-2">
                <div className="flex items-center gap-1">
                    <Bed className="w-5 h-5 text-gray-700" /> {property.spesifikasi?.kamarTidur ?? 0}
                </div>
                <div className="flex items-center gap-1">
                    <Bath className="w-5 h-5 text-gray-700" /> {property.spesifikasi?.kamarMandi ?? 0}
                </div>
                <div className="flex items-center gap-1">
