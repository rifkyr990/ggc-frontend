...
export default function KarirPage() {
    ...
    useEffect(() => {
        fetch(BACKENDLESS_URL)
            .then((res) => {
                if (!res.ok) throw new Error("Gagal mengambil data karir");
                return res.json();
            })
            .then((data) => {
                setJobs(data);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-white via-green-50 to-yellow-100 pb-16">
            ...
            >
                ...
                <div className="relative z-10 flex flex-col items-center justify-center h-full w-full px-4">
                    ...
                    <p className="text-lg md:text-xl text-white/90 max-w-2xl text-center">
                        Bergabunglah bersama kami membangun masa depan properti Indonesia. Temukan peluang karir terbaik di industri real estate modern, inovatif, dan penuh prestise.
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-4 md:px-0">
                <h2 className="text-3xl font-bold text-[#BFA14A] mb-8 text-center drop-shadow-sm">Lowongan Tersedia</h2>
                {loading ? (
                    <div className="flex flex-col items-center justify-center min-h-[40vh]">
                        <div className="w-16 h-16 border-4 border-[#BFA14A]/30 border-t-[#BFA14A] rounded-full animate-spin mb-6"></div>
                        <div className="text-lg font-medium text-[#BFA14A] tracking-wide">Mengambil data lowongan...</div>
                    </div>
                ) : error ? (
                    <div className="flex flex-col items-center justify-center min-h-[40vh]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                        {jobs.length === 0 && (
                            <div className="col-span-full text-center text-gray-500">Belum ada lowongan tersedia.</div>
                        )}
                        {jobs.map((job, idx) => (
                            <div
                                key={job.objectId || idx}
                            ...
                            >
                                ...
                                <div className="relative h-48 w-full overflow-hidden">
                                    <img
                                        src={job.imageUrl || "/image/bg-hero/4.png"}
                                        alt={job.title}
                                ...
                                <div className="p-7 flex flex-col gap-3">
                                    ...
                                    <div className="flex flex-wrap gap-2 text-xs mb-2">
                                        <span className="bg-[#BFA14A]/10 text-[#BFA14A] px-2 py-1 rounded-full">{job.location || "Lokasi fleksibel"}</span>
                                        <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded-full">{job.type || "Full Time"}</span>
                                    </div>
                                    <p className="text-green-800 text-base mb-2 line-clamp-3">
                                        {job.description?.slice(0, 120) || "Deskripsi tidak tersedia."}
                                    </p>
                                    <div className="mt-auto flex justify-end">
                                        <a
                                            href={`https://wa.me/6282142991064?text=Halo%20HRD%2C%20saya%20ingin%20melamar%20untuk%20posisi%20${encodeURIComponent(job.title)}.`}
                                            target="_blank"
