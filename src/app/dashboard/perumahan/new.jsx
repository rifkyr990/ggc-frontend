...
const Page = () => {
  ...
  const handleChange = (e) => {
    ...
    } else if (name === 'fasilitas' && e.target.multiple) {
      const selected = Array.from(e.target.selectedOptions, option => option.value);
      setFormData((prev) => ({ ...prev, fasilitas: selected }));
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    form.append('fasilitasIds', JSON.stringify(formData.fasilitasIds));
      ...
      <main className="md:ml-64 p-6 bg-gray-100 min-h-screen">
        ...
        <section className="bg-white p-4 sm:p-6 rounded-lg shadow mb-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4" encType="multipart/form-data">
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="deskripsi" className="text-sm text-gray-700">Deskripsi</label>
              <textarea
                id="deskripsi"
                name="deskripsi"
        ...
        <section className="relative overflow-x-auto shadow-md rounded-lg">
                ...
                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex gap-2">
                      <Button className="bg-blue-500 text-white px-5 py-2 rounded" onClick={() => handleEdit(item)}>Edit</Button>
                      <Button className="bg-red-600 text-white px-5 py-2 rounded" onClick={() => handleDelete(item.id)}>Hapus</Button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="my-4 flex justify-center items-center gap-4">
            ...
            <Button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="bg-gray-200 px-3 py-1">
              Next
            </Button>
          </div>
        </section>
