...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nama || !formData.iconUrl) {
  ...
  const handleEdit = (item) => {
    setFormData({ nama: item.nama, iconUrl: null })
    setEditId(item.id)
  }
        ...
        <section className="bg-white p-4 rounded-lg shadow mb-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex-1">
              <label htmlFor="nama" className="mb-2">Nama Fasilitas:</label>
              <input
                type="text"
                name="nama"
                id="nama"
            ...
            <div className="flex-1">
              <label htmlFor="iconUrl" className="mb-2">Upload Icon:</label>
              <input
                type="file"
                name="iconUrl"
                accept="image/*"
              ...
              {fasilitas.map((item) => (
                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">
                    <img src={item.iconUrl} alt={item.nama} className="w-8 h-8" />
                  </td>
                  <td className="px-4 py-2">
